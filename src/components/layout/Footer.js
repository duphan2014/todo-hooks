...
class Footer extends React.Component {
    ...
    submitThemeColor(color){
        if(color){
            ...
            this.props.saveColorTheme(color); // set up by mapDispatchToProp in containers/Footer.js
        }
    };

    componentWillReceiveProps(nextprops){
        console.log('UNSAFE_componentWillReceiveProps: ' + JSON.stringify(nextprops));
        document.documentElement.style.setProperty("--main-color", nextprops.themeColor.color); //set up by mapStateToProp in containers/Footer.js
    }
    render(){
        return (
